...
// Frontend origin for CORS (configurable for prod)
...
const connectDB = async () => {
  try {
    ...
    console.error("2. MongoDB Atlas IP whitelist (add 0.0.0/0 for testing)");
    console.error("3. Database user credentials");
    console.error("4. If using MongoDB Atlas, verify your IP is whitelisted");
...
app.post('/auth/login', async (req, res) => {
  ...
  loginAttempts.forEach((attempts, ip) => {
    loginAttempts.set(ip, attempts.filter(time => now - time < LOGIN_TIMEOUT));
  });

  // Check for too many login attempts
  const attempts = loginAttempts.get(ip) || [];
  if (attempts.length >= MAX_LOGIN_ATTEMPTS) {
    const retryAfter = Math.ceil((attempts[0] + LOGIN_TIMEOUT - now) / 1000);
  ...
  try {
    ...
    if (!isMatch) {
      ...
      return res.status(401).json({ 
        success: false,
        error: "Invalid login credentials",
        remainingAttempts: 5 - (user.failedLoginAttempts || 0)
      });
    }
